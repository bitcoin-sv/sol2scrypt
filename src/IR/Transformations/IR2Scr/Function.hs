...
instance ToScryptTransformable IFunction (Scr.Function Ann) where
  _toScrypt (IR.Function fid (ParamList ps) (IR.Block stmts) rt vis static) = funcRetGuard $
    Scr.Function
      (_toScrypt fid)
      (map _toScrypt ps)
...
funcRetGuard f@(Scr.Function _ _ _ (RegularBody stmts _ _) vis _ _) =
  case reverse stmts of
    ...
    s -> error $ "function `" ++  unName (Scr.funcName f) ++ "` have inconsistant visibility `" ++ show vis ++ "` with the last statement `" ++ if null s then "[]" else show (head s) ++ "`"
