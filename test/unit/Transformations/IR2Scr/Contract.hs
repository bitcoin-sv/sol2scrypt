...
spec = testSpec "Contract" $ do
  describe "instance ToScryptTransformable IContractBodyElement" $ do
    it "should transform IR `StateVariableDeclaration` to sCrypt Param correctly" $ do
      r <- transform2Scrypt (IR.StateVariableDeclaration (IR.StateVariable (IR.Identifier "a") (IR.ElementaryType IR.Int) IR.Public Nothing False False))
      r `shouldBe` Scr.Param (TypeAnn Scr.Int nil) (NameAnn "a" nil) (Const False) Nothing Scr.Public (IsStateProp True) nil

    it "should transform IR `StateVariableDeclaration` to sCrypt Param correctly" $ do
      r <- transform2Scrypt (IR.StateVariableDeclaration (IR.StateVariable (IR.Identifier "a") (IR.ElementaryType IR.Int) IR.Public Nothing False True))
    ...
    it "should transform IR `StateVariableDeclaration` to sCrypt Static correctly" $ do
      r <- transform2Scrypt (IR.StateVariableDeclaration (IR.StateVariable (IR.Identifier "a") (IR.ElementaryType IR.Int) IR.Public (Just (LiteralExpr (IR.IntLiteral {IR.isHex = False, IR.intVal = 8}))) True False))
