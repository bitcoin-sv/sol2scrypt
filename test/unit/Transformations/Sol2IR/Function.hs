...
spec = testSpec "instance ToIRTransformable Contractpart IFunction'" $ do
  ...
  let expFuncWithoutRet_G1 vis = Just (Function {funcName = IR.Identifier "add", funcParams = ParamList [Param {paramType = ElementaryType Int, paramName = IR.Identifier "x"},Param {paramType = ElementaryType Int, paramName = IR.Identifier "y"}], funcBody = IR.Block [ReturnStmt {retExpr = LiteralExpr (BoolLiteral True)}], funcReturn = ElementaryType Bool, funcVisibility = vis})
      ...
      expFuncWithoutRet_G4 vis = Just (Function {funcName = IR.Identifier  "add", funcParams = ParamList [Param {paramType = ElementaryType Int, paramName = IR.Identifier  "x"},Param {paramType = ElementaryType Int, paramName = IR.Identifier  "y"},Param {paramType = BuiltinType "SigHashPreimage", paramName = IR.Identifier  "txPreimage"}], funcBody = IR.Block [RequireStmt {verifyExpr = IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr {instanceExpr = IdentifierExpr (IR.Identifier  "Tx"), member = IR.Identifier  "checkPreimage"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}},DeclareStmt [Just (Param {paramType = ElementaryType Bytes, paramName = IR.Identifier  "outputScript"})] [IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr {instanceExpr = IdentifierExpr (IR.Identifier  "this"), member = IR.Identifier  "getStateScript"}, funcParamExprs = []}],DeclareStmt [Just (Param {paramType = ElementaryType Bytes, paramName = IR.Identifier  "output"})] [IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr {instanceExpr = IdentifierExpr (IR.Identifier  "Utils"), member = IR.Identifier  "buildOutput"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "outputScript"),IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr {instanceExpr = IdentifierExpr (IR.Identifier  "SigHash"), member = IR.Identifier  "value"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}]}],RequireStmt {verifyExpr = BinaryExpr {binaryOp = IR.Equal, lExpr = IR.FunctionCallExpr {funcExpr = IdentifierExpr (IR.Identifier  "hash256"), funcParamExprs = [IdentifierExpr (IR.Identifier  "output")]}, rExpr = IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr {instanceExpr = IdentifierExpr (IR.Identifier  "SigHash"), member = IR.Identifier  "hashOutputs"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}}}], funcReturn = ElementaryType Bool, funcVisibility = vis})
      expFuncWithOnlyTypeRet_G4 vis = Just (Function {funcName = IR.Identifier  "add", funcParams = ParamList [Param {paramType = ElementaryType Int, paramName = IR.Identifier  "x"},Param {paramType = ElementaryType Int, paramName = IR.Identifier  "y"},Param {paramType = BuiltinType "SigHashPreimage", paramName = IR.Identifier  "txPreimage"},Param {paramType = ElementaryType Int, paramName = IR.Identifier  "retVal"}], funcBody = IR.Block [RequireStmt {verifyExpr = IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "Tx"), member = IR.Identifier  "checkPreimage"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}},DeclareStmt [Just (Param {paramType = ElementaryType Bytes, paramName = IR.Identifier  "outputScript"})] [IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "this"), member = IR.Identifier  "getStateScript"}, funcParamExprs = []}],DeclareStmt [Just (Param {paramType = ElementaryType Bytes, paramName = IR.Identifier  "output"})] [IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "Utils"), member = IR.Identifier  "buildOutput"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "outputScript"), IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "SigHash"), member = IR.Identifier  "value"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}]}],RequireStmt {verifyExpr = BinaryExpr {binaryOp = IR.Equal , lExpr = IR.FunctionCallExpr {funcExpr = IdentifierExpr (IR.Identifier  "hash256"), funcParamExprs = [IdentifierExpr (IR.Identifier  "output")]}, rExpr = IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "SigHash"), member = IR.Identifier  "hashOutputs"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}}},RequireStmt {verifyExpr = BinaryExpr {binaryOp = IR.Equal , lExpr = BinaryExpr {binaryOp = IR.Add , lExpr = IdentifierExpr (IR.Identifier  "x"), rExpr = IdentifierExpr (IR.Identifier  "y")}, rExpr = IdentifierExpr (IR.Identifier  "retVal")}}], funcReturn = ElementaryType Bool, funcVisibility = vis})
      expFuncWithNamedRet_G4 vis = Just (Function {funcName = IR.Identifier  "add", funcParams = ParamList [Param {paramType = ElementaryType Int, paramName = IR.Identifier  "x"},Param {paramType = ElementaryType Int, paramName = IR.Identifier  "y"},Param {paramType = BuiltinType "SigHashPreimage", paramName = IR.Identifier  "txPreimage"},Param {paramType = ElementaryType Int, paramName = IR.Identifier  "_z"}], funcBody = IR.Block [DeclareStmt [Just (Param {paramType = ElementaryType Int, paramName = IR.Identifier  "z"})] [LiteralExpr (IntLiteral {isHex = False, intVal = 0})],AssignStmt [Just (IR.Identifier  "z")] [BinaryExpr {binaryOp = IR.Add , lExpr = IdentifierExpr (IR.Identifier  "x"), rExpr = IdentifierExpr (IR.Identifier  "y")}],RequireStmt {verifyExpr = IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "Tx"), member = IR.Identifier  "checkPreimage"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}},DeclareStmt [Just (Param {paramType = ElementaryType Bytes, paramName = IR.Identifier  "outputScript"})] [IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "this"), member = IR.Identifier  "getStateScript"}, funcParamExprs = []}],DeclareStmt [Just (Param {paramType = ElementaryType Bytes, paramName = IR.Identifier  "output"})] [IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "Utils"), member = IR.Identifier  "buildOutput"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "outputScript"), IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "SigHash"), member = IR.Identifier  "value"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}]}],RequireStmt {verifyExpr = BinaryExpr {binaryOp = IR.Equal, lExpr = IR.FunctionCallExpr {funcExpr = IdentifierExpr (IR.Identifier  "hash256"), funcParamExprs = [IdentifierExpr (IR.Identifier  "output")]}, rExpr = IR.FunctionCallExpr {funcExpr = IR.MemberAccessExpr  {instanceExpr = IdentifierExpr (IR.Identifier  "SigHash"), member = IR.Identifier  "hashOutputs"}, funcParamExprs = [IdentifierExpr (IR.Identifier  "txPreimage")]}}},RequireStmt {verifyExpr = BinaryExpr {binaryOp = IR.Equal , lExpr = IdentifierExpr (IR.Identifier  "z"), rExpr = IdentifierExpr (IR.Identifier  "_z")}}], funcReturn = ElementaryType Bool, funcVisibility = vis})
