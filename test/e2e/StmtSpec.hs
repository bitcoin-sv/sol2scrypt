  ...
  describe "#SimpleStatementExpression" $ do
    describe "#NumberLiteral" $ do
      it "should transpile Solidity `NumberLiteralHex` correctly" $ do
        tr :: TranspileResult Sol.Statement IStatement' (Maybe (Scr.Statement IExpr)) <- transpile "0x123a;"
        scryptCode tr `shouldBe` "0x123a;"
        tr1 :: TranspileResult Sol.Statement IStatement' (Maybe (Scr.Statement IExpr)) <- transpile "256;"
