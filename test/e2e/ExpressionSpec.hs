        ...
        describe "when the left expr is a mapping-typed var" $ do
          let itSBExpr leftExpr rightExpr scrypt = 
                it "should transpile correctly" $ do
                  let mapSym = Symbol (IR.Identifier leftExpr) (Mapping (ElementaryType Address) (ElementaryType IR.Int)) False False False
                  ...
                  tr :: TranspileResult (Expression SourceRange) IExpression' (Maybe (Expr Ann)) <- 
                          transpile' (TransformState initEnv Nothing Map.empty [] Map.empty [] 0 [] False Set.empty Set.empty [] False False 0) sol ""
                  scryptCode tr `shouldBe` scrypt
